...
use rwm::drw::{fontset_create, Drw};
...
fn checkotherwm(dpy: *mut Display) {
...
fn setup(dpy: *mut Display) -> State {
    log::trace!("setup");
    unsafe {
        ...
        let drw = drw::create(dpy, SCREEN, ROOT, sw as u32, SH as u32);
        if fontset_create(drw, &CONFIG.fonts).is_null() {
        ...
        WMCHECKWIN =
            xlib::XCreateSimpleWindow(state.dpy, ROOT, 0, 0, 1, 1, 0, 0, 0);
...
fn updatesystray(state: &mut State) {
    unsafe {
        ...
        let sw = textw(state.drw, addr_of!(STEXT) as *const _) - LRPAD
        ...
        if SYSTRAY.is_null() {
            ...
            XMapRaised(state.dpy, (*SYSTRAY).win);
            XSetSelectionOwner(
                state.dpy,
                state.netatom[Net::SystemTray as usize],
                (*SYSTRAY).win,
        ...
        XMapWindow(state.dpy, (*SYSTRAY).win);
        XMapSubwindows(state.dpy, (*SYSTRAY).win);
        // redraw background
        XSetForeground(
            state.dpy,
            (*state.drw).gc,
...
fn drawbar(state: &mut State, m: *mut Monitor) {
    ...
    unsafe {
        ...
        if m == SELMON {
            ...
            drw::setscheme(state.drw, *SCHEME.add(Scheme::Norm as usize));
            tw = textw(state.drw, addr_of!(STEXT) as *const _) - LRPAD / 2 + 2; // 2px right padding
            drw::text(
                state.drw,
                (*m).ww - tw - stw as i32,
                0,
        ...
        for (i, tag) in CONFIG.tags.iter().enumerate() {
            ...
            let w = textw(state.drw, text.as_ptr());
            drw::setscheme(
                state.drw,
                *SCHEME.add(
        ...
        let w = textw(state.drw, (*m).ltsymbol.as_ptr());
        drw::setscheme(state.drw, *SCHEME.add(Scheme::Norm as usize));
        x = drw::text(
            state.drw,
            x,
        ...
        if w > state.bh {
            if !(*m).sel.is_null() {
                ...
                drw::setscheme(state.drw, *SCHEME.add(Scheme::Norm as usize));
                drw::rect(state.drw, x, 0, w as u32, state.bh as u32, 1, 1);
            }
        }
        drw::map(
            state.drw,
    ...
    unsafe {
        ...
        while !m.is_null() {
            ...
            (*m).barwin = xlib::XCreateWindow(
                ...
                state.bh as c_uint,
                0,
                xlib::XDefaultDepth(state.dpy, SCREEN),
                CopyFromParent as c_uint,
                xlib::XDefaultVisual(state.dpy, SCREEN),
...
fn getrootptr(state: &mut State, x: *mut c_int, y: *mut c_int) -> c_int {
    unsafe {
        ...
        let mut dummy = 0;
        xlib::XQueryPointer(
            state.dpy, ROOT, &mut dummy, &mut dummy, x, y, &mut di, &mut di,
            &mut dui,
        )
    }
}
...
fn cleanup(mut state: State) {
    ...
    unsafe {
        ...
        if CONFIG.showsystray {
            XUnmapWindow(state.dpy, (*SYSTRAY).win);
            XDestroyWindow(state.dpy, (*SYSTRAY).win);
            libc::free(SYSTRAY.cast());
        }
        ...
        let State { dpy, drw, cursors, .. } = state;
...
fn run(state: &mut State) {
    unsafe {
        ...
        while RUNNING && xlib::XNextEvent(state.dpy, ev.as_mut_ptr()) == 0 {
...
fn main() {
    ...
    let _ = CONFIG;
    let dpy = unsafe { xlib::XOpenDisplay(std::ptr::null_mut()) };
    if dpy.is_null() {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        unsafe { XCON = Box::into_raw(Box::new(xcon)) };
    }
